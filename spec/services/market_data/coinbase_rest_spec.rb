...
RSpec.describe MarketData::CoinbaseRest, type: :service do
  ...
  describe "#fetch_candles" do
    ...
    let(:candle_data) { [[1640995200, 50000, 51000, 49500, 50500, 100]] }
    ...
    context "with basic parameters" do
      it "makes GET request with correct path" do
        ...
        expect(mock_connection).to have_received(:get).with("/products/#{product_id}/candles", hash_including(granularity: 3600))
      end
    ...
    context "with hash response containing candles" do
      ...
      it "converts hash format to array format" do
        ...
        expected_data = [[1640995200, "50000", "51000", "49500", "50500", "100"]]
    ...
    context "with error response" do
      ...
      it "raises error with message" do
        ...
        expect {
          service.fetch_candles(product_id: product_id)
        }.to raise_error("API Error: Invalid product")
      end
    end
  end
  ...
  describe "#authenticated_get" do
    ...
    before do
      allow(Time).to receive(:now).and_return(Time.at(1640995200)) # Fixed timestamp for testing
  ...
  describe "#upsert_1h_candles" do
    ...
    it "creates candles in database" do
      expect(Candle).to receive(:create_or_find_by).with(
        symbol: product_id,
        timeframe: "1h",
        timestamp: Time.at(1640995200).utc
      )
  ...
  describe "#fetch_candles_in_chunks" do
    ...
    let(:end_time) { Time.now }
    let(:chunk_data) { [[1640995200, 50000, 51000, 49500, 50500, 100]] }
