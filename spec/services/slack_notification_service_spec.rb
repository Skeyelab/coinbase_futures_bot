...
RSpec.describe SlackNotificationService, type: :service do
  ...
  describe '.position_update' do
    let(:position) { create(:position, product_id: 'ETH-USD', side: 'long', size: 1.0, entry_price: 3000.0, pnl: 150.0) }
  ...
  describe 'error handling' do
    it 'handles Slack API errors gracefully' do
      ...
      expect(Rails.logger).to receive(:error).with(include('Slack API Error'))
    ...
    it 'retries on API errors with exponential backoff' do
      allow(mock_client).to receive(:chat_postMessage)
        .and_raise(Slack::Web::Api::Errors::SlackError.new('rate_limited'))
        .twice
        .and_return(true)
    ...
    it 'gives up after max retries' do
      ...
      expect(Rails.logger).to receive(:error).with(include('Failed to send message after 3 retries'))
