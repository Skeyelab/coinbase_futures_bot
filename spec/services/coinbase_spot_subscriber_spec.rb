...
RSpec.describe MarketData::CoinbaseSpotSubscriber, type: :service do
  def build_logger(io)
  ...
  it "logs ticker messages (legacy flat schema)" do
    io = StringIO.new
    logger = build_logger(io)
    service = described_class.new(product_ids: "BTC-USD", logger: logger)

    message = OpenStruct.new(data: { type: "ticker", product_id: "BTC-USD", price: "123.45", time: "2024-01-01T00:00:00Z" }.to_json)
