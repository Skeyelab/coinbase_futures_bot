  ...
  describe "#check_tp_sl_triggers" do
    ...
    context "with real price data from ticks" do
      before do
        ...
        create(:tick, product_id: "BIT-29AUG25-CDE", price: 51_100.0, observed_at: 1.minute.ago)
    ...
    context "with real price data from candles" do
      before do
        ...
        create(:candle,
          ...
          timeframe: "1m",
          close: 48_800.0, # Below stop loss
      ...
      it "returns positions with triggered stop loss" do
        ...
        expect(long_trigger[:current_price]).to eq(48_800.0)
        expect(long_trigger[:target_price]).to eq(49_000.0)
      end
    end
    ...
    context "when no triggers are hit" do
      before do
        ...
        create(:tick, product_id: "BIT-29AUG25-CDE", price: 50_000.0, observed_at: 1.minute.ago)
  ...
  describe "#get_current_prices" do
    ...
    context "with candle data fallback" do
      ...
      it "returns current prices from candles when ticks unavailable" do
        result = manager.get_current_prices
        expect(result[position.id]).to eq(50_500.0)
      end
    end
    ...
    context "with no price data" do
      it "returns entry price as fallback" do
        result = manager.get_current_prices
        expect(result[position.id]).to eq(50_000.0)
      end
    end
  end
