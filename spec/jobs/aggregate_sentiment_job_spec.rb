...
RSpec.describe AggregateSentimentJob, type: :job do
  it "creates aggregates for 15m window and computes z_score" do
    ...
    3.times do |i|
      SentimentEvent.create!(source: "cryptopanic", symbol: "BTC-USD-PERP", published_at: win_start + i.minutes, raw_text_hash: "h#{i}", score: 0.5)
    end

    described_class.perform_now(now: now)

    agg = SentimentAggregate.where(symbol: "BTC-USD-PERP", window: "15m").order(window_end_at: :desc).first
