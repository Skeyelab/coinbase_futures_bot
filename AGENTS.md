...
- Position side conventions are uppercase `LONG` / `SHORT` in `Position`, but signal side conventions are lowercase-like trading verbs (`long`, `short`, `buy`, `sell`, `unknown`) in `SignalAlert`.
