...
module Trading
  ...
  class SwingPositionManager
    ...
    def archive_completed_trades(days_old: 7)
      cutoff_time = days_old.days.ago
      completed_trades = Position.swing_trading.closed.where("close_time < ? AND close_time > ?", cutoff_time, 90.days.ago)
    ...
    def calculate_swing_risk_metrics(summary)
      ...
      metrics = {
        ...
        avg_hold_time_hours: summary[:positions].sum { |p| p[:duration_hours] || 0 } / summary[:total_positions],
      ...
      if summary[:positions_by_asset].any?
        ...
        metrics[:max_asset_concentration] =
          (summary[:total_exposure] > 0) ? (max_asset_exposure / summary[:total_exposure]) : 0
      end
    ...
    def archive_trade_summary(position)
      ...
      @logger.info("Archived swing trade summary: #{trade_summary.to_json}")
