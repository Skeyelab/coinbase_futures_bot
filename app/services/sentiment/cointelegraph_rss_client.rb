...
module Sentiment
  class CointelegraphRssClient < BaseNewsClient
    RSS_URL = "https://cointelegraph.com/rss"

    def initialize(rss_url: ENV["COINTELEGRAPH_RSS_URL"], logger: Rails.logger)
      super(logger: logger)
    ...
    def fetch_recent(max_pages: 2)
      @logger.debug("Cointelegraph RSS: Fetching from #{@rss_url}")

      response = fetch_with_redirects(@rss_url)

      unless response.is_a?(Net::HTTPSuccess)
        @logger.error("Cointelegraph RSS: HTTP error #{response.code}: #{response.message}")
        return []
      end
      ...
      @logger.info("Cointelegraph RSS: Successfully fetched #{items.size} items")

      # Track successful data fetching
      SentryHelper.add_breadcrumb(
        message: "Cointelegraph RSS data fetched successfully",
        category: "sentiment",
        level: "info",
        data: {
          service: "cointelegraph_rss",
      ...
      @logger.error("Cointelegraph RSS HTTP error: #{e.class} #{e.message}")
      handle_error(e, "http_error")
      []
    rescue REXML::ParseException => e
      @logger.error("Cointelegraph RSS XML parse error: #{e.message}")
      handle_error(e, "xml_parse_error")
      []
    rescue => e
      @logger.error("Cointelegraph RSS fetch failed: #{e.class} #{e.message}")
    ...
    def fetch_with_redirects(url, limit = 5)
      ...
      when Net::HTTPRedirection
        ...
        @logger.debug("Cointelegraph RSS: Following redirect to #{location}")
    ...
    def handle_error(exception, error_type)
      Sentry.with_scope do |scope|
        scope.set_tag("service", "cointelegraph_rss")
