...
class SlackController < ApplicationController
  ...
  def handle_app_mention(event)
    ...
    if text.include?('help')
      response_text = "Hi! 👋 Use these slash commands to interact with me:\n" \
                      "• `/bot-status` - Bot status\n" \
                      "• `/bot-positions` - Current positions\n" \
                      "• `/bot-pnl` - PnL report\n" \
                      "• `/bot-help` - Full command list"
    elsif text.include?('status')
      response_text = "Use `/bot-status` to get the current bot status with detailed information."
    else
      response_text = "I'm here! Use `/bot-help` to see all available commands."
    end
