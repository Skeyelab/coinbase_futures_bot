...
namespace :coverage do
  ...
  desc "Run tests with coverage using parallel RSpec"
  task parallel: :environment do
    puts "🚀 Running parallel tests with SimpleCov coverage..."
    puts "This will use parallel execution for faster test runs with coverage reporting."

    # Clean any existing coverage files
    puts "🧹 Cleaning existing coverage files..."
    system("rm -rf coverage/")

    # Set coverage environment variable
    ENV["COVERAGE"] = "true"
    ENV["PARALLEL_COVERAGE"] = "true"

    # Run parallel tests with coverage using parallel_tests
    puts "⚡ Running parallel RSpec with coverage..."
    system("bundle exec parallel_test spec/ --type rspec")

    if $?.success?
      puts "\n✅ Parallel tests completed successfully!"

      # Collate coverage results if parallel execution was used
      if ENV["PARALLEL_COVERAGE"] == "true"
        puts "📊 Collating coverage results from parallel processes..."
        require "simplecov"
...
# Default coverage task (uses parallel by default for faster execution)
task coverage: "coverage:parallel"
