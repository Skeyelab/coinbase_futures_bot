...
namespace :real_time do
  ...
  task status: :environment do
    ...
    puts "Trading Pairs:"
    puts "  BTC-USD: #{btc_pair ? (btc_pair.enabled? ? "✓ enabled" : "✗ disabled") : "✗ not found"}"
    puts "  ETH-USD: #{eth_pair ? (eth_pair.enabled? ? "✓ enabled" : "✗ disabled") : "✗ not found"}"
    puts
  ...
  def calculate_current_eth_capacity
    ...
    eth_price = 3200.0 # Approximate ETH price
    contract_size = 10.0 # $10 per ETH contract
    ...
    max_contracts = [contracts_per_trade * 5, 20].min # 5 concurrent positions, max 20
